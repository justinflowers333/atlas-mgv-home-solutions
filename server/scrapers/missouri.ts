...
// ─── BANKRUPTCY — Western District of MO (ecf.mowb.uscourts.gov) ─────────────
export async function scrapeBankruptcy(fromDate: string, toDate: string): Promise<Lead[]> {
  ...
  try {
    ...
    for (const item of items) {
      ...
      leads.push({
        id: makeId("MO", STATE, "Bankruptcy", caseNum),
        county: "MO",
        state: STATE,
...
// ─── CODE VIOLATIONS — Missouri municipal portals ─────────────────────────

// ─── PROBATE — Missouri probate court filings ────────────────────────────────
export async function scrapeProbate(fromDate: string, toDate: string): Promise<Lead[]> {
  const leads: Lead[] = [];
  try {
    const url = `https://www.courtlistener.com/api/rest/v4/dockets/?court=mowb&date_filed__gte=${fromDate}&date_filed__lte=${toDate}&nature_of_suit=110&order_by=-date_filed&page_size=50`;
    ...
    if (res.ok) {
      ...
      for (const r of (data?.results || []) as Record<string, unknown>[]) {
        ...
        leads.push({
          id: makeId("PROB", caseNum || caseName, "MO", "probate"),
          county: "MO", state: "MO", lead_type: "Probate",
          ...
          description: `Missouri Probate — ${caseName || caseNum}`,
  ...
  } catch (e) { console.error("[MO] Probate error:", e); }
  return leads;
}
...
export async function scrapeCodeViolations(fromDate: string, toDate: string): Promise<Lead[]> {
  const leads: Lead[] = [];
  try {
    const url = `https://www.courtlistener.com/api/rest/v4/dockets/?court=mowd&date_filed__gte=${fromDate}&date_filed__lte=${toDate}&nature_of_suit=440&order_by=-date_filed&page_size=50`;
    const res = await fetchWithRetry(url, { headers: { "User-Agent": "Atlas/1.0", Accept: "application/json" } });
    if (res.ok) {
      const data = await res.json() as { results?: unknown[] };
      for (const r of (data?.results || []) as Record<string, unknown>[]) {
  ...
  try {
    ...
    if (rssRes.ok) {
      ...
      for (const item of items) {
        ...
        leads.push({ id: makeId("DIV", title, "MO", "divorce"), county: "MO", state: "MO", lead_type: "Divorce", owner_name: title.split(/\s+v\.?\s+/i).join(" & "), address: null, city: null, zip: null, mailing_address: null, mailing_city: null, mailing_state: null, mailing_zip: null, case_number: null, filing_date: pubDate ? formatDate(new Date(pubDate).toISOString().slice(0,10)) : formatDate(fromDate), assessed_value: null, tax_year: null, lender: null, loan_amount: null, sale_date: null, sale_amount: null, description: `Divorce / Eviction — ${title}`, source_url: link || "https://ecf.mowd.uscourts.gov/cgi-bin/rss_outside.pl", raw_data: JSON.stringify({ title, pubDate, desc }) });
      }
    }
  } catch (e) { console.error("[MO] Divorce/Eviction error:", e); }
  return leads;
}

// ─── OUT-OF-STATE OWNERS — CourtListener Missouri ─────────────────────────
export async function scrapeOutOfStateOwners(fromDate: string, toDate: string): Promise<Lead[]> {
  const leads: Lead[] = [];
  try {
    const url = `https://www.courtlistener.com/api/rest/v4/dockets/?court=mowd&date_filed__gte=${fromDate}&date_filed__lte=${toDate}&nature_of_suit=290&order_by=-date_filed&page_size=50`;
    const res = await fetchWithRetry(url, { headers: { "User-Agent": "Atlas/1.0", Accept: "application/json" } });
    if (res.ok) {
      const data = await res.json() as { results?: unknown[] };
      for (const r of (data?.results || []) as Record<string, unknown>[]) {
        ...
        const filedDate = String(r.date_filed || "");
        if (!caseName && !caseNum) continue;
        leads.push({ id: makeId("OOS", caseNum || caseName, "MO", "oos"), county: "MO", state: "MO", lead_type: "Out-of-State Owner", owner_name: caseName || null, address: null, city: null, zip: null, mailing_address: null, mailing_city: null, mailing_state: null, mailing_zip: null, case_number: caseNum || null, filing_date: formatDate(filedDate), assessed_value: null, tax_year: null, lender: null, loan_amount: null, sale_date: null, sale_amount: null, description: `Out-of-State Owner — ${caseName || caseNum}`, source_url: r.absolute_url ? `https://www.courtlistener.com${r.absolute_url}` : "https://www.courtlistener.com/", raw_data: JSON.stringify({ caseName, caseNum, filedDate }) });
      }
    }
  } catch (e) { console.error("[MO] Out-of-State Owners error:", e); }
  return leads;
}

// ─── VACANT / ABANDONED — Missouri PACER BK RSS ──────────────────────────
export async function scrapeVacantAbandoned(fromDate: string, toDate: string): Promise<Lead[]> {
  const leads: Lead[] = [];
  try {
    const rssRes = await fetchWithRetry("https://ecf.mowb.uscourts.gov/cgi-bin/rss_outside.pl");
    if (rssRes.ok) {
      const xml = await rssRes.text();
      const items = xml.match(/<item>[\s\S]*?<\/item>/g) || [];
      for (const item of items) {
        ...
        leads.push({ id: makeId("VAC", title, "MO", "vacant"), county: "MO", state: "MO", lead_type: "Vacant/Abandoned", owner_name: title.split(/\s+v\.?\s+/i)[0]?.trim() || title, address: null, city: null, zip: null, mailing_address: null, mailing_city: null, mailing_state: null, mailing_zip: null, case_number: null, filing_date: pubDate ? formatDate(new Date(pubDate).toISOString().slice(0,10)) : formatDate(fromDate), assessed_value: null, tax_year: null, lender: null, loan_amount: null, sale_date: null, sale_amount: null, description: `Vacant/Abandoned — Chapter 7 — ${title}`, source_url: link || "https://ecf.mowb.uscourts.gov/cgi-bin/rss_outside.pl", raw_data: JSON.stringify({ title, pubDate, desc }) });
      }
    }
  } catch (e) { console.error("[MO] Vacant/Abandoned error:", e); }
  return leads;
}
